      ...
      module Internal
        ...
        def self.parse(data)
          ...
          data['nodes'].each do |node|
            binding.pry
            map.nodes[node['id']] = Berlin::AI::Node::Internal.parse(node.merge(types[node['type']]))
          end
          ...
          data['paths'].each do |path|
            binding.pry
            map.nodes[path['from']].link_to(map.nodes[path['to']])

            # Don't forget! If the map is not directed, we must create the reverse link!
            map.nodes[path['to']].link_to(map.nodes[path['from']]) unless directed?
          end
